...
class TestCubic:
    ...
    def test_init(self):
        ...
        s = Cubic(r=5.0, d_next=0.0, b=[0.01], mat2="bk7")
        assert s.b_degree == 1
    ...
    def test_sag_nonzero_off_center(self):
        ...
        s = Cubic(r=5.0, d_next=0.0, b=[0.1], mat2="bk7")
        x = torch.tensor(1.0)
        y = torch.tensor(0.0)
        z = s.sag(x, y)
        assert z.abs().item() > 0
    ...
    def test_derivatives(self):
        ...
        s = Cubic(r=5.0, d_next=0.0, b=[0.1, 0.01], mat2="bk7")
        x = torch.tensor(1.0)
        y = torch.tensor(1.0)
    ...
    def test_surf_dict(self):
        ...
        s = Cubic(r=5.0, d_next=0.0, b=[0.01, 0.001, 0.0001], mat2="bk7")
        d = s.surf_dict()
        assert d["type"] == "Cubic"
...
class TestThinLens:
    ...
    def test_init(self):
        ...
        tl = ThinLens(r=5.0, d_next=0.0, f=50.0)
    ...
    def test_refract_converges(self):
        ...
        tl = ThinLens(r=10.0, d_next=0.0, f=50.0)
...
class TestQTypeFreeform:
    ...
    def test_sag_center_zero(self):
        """Sag at center should be zero."""
        s = QTypeFreeform(r=5.0, d_next=0.0, c=0.1, k=0.0, qm=[0.001], mat2="bk7")
        x = torch.tensor(0.0)
        y = torch.tensor(0.0)
        z = s.sag(x, y)
        assert z.abs().item() < 1e-6
    ...
    def test_reduces_to_conic_when_qm_zero(self):
        ...
        c = 0.05
        k = -1.0
        s = QTypeFreeform(r=5.0, d_next=0.0, c=c, k=k, qm=[0.0, 0.0], mat2="bk7")
        x = torch.tensor(1.0)
        y = torch.tensor(0.0)
        sag = s.sag(x, y)
...
class TestSpiral:
    ...
    def test_init(self):
        ...
        s = Spiral(r=5.0, d_next=0.0, c1=0.1, c2=0.05, mat2="bk7")
        assert s.r == 5.0
    ...
    def test_sag_at_origin(self):
        ...
        s = Spiral(r=5.0, d_next=0.0, c1=0.2, c2=0.1, mat2="bk7")
        x = torch.tensor(0.0)
        y = torch.tensor(0.0)
        z = s.sag(x, y)
