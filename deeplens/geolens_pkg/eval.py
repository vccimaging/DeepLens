...
class GeoLensEval:
    ...
    def calc_chief_ray(
        ...
        *,
        ...
        samples a finite bundle, this method returns the sampled real ray with
        the largest stop-proximity weight rather than a synthetic bundle
        centroid. The selection residual scales as ``~1/sqrt(num_rays)`` of
        the stop radius; raise ``num_rays`` when tighter stop centring is
        needed (e.g. relative distortion at small field angles, where the
        residual is amplified by the small ideal image height).
        ...
        chief_ray, chief_path = self._extract_chief_from_bundle(ray, ray_path)
        if record:
            return chief_ray, chief_path
        return chief_ray

    @torch.no_grad()
    def _extract_chief_from_bundle(self, ray, ray_path=None):
        """Select the stop-centred chief ray from a traced, stop-weighted bundle.

        Args:
            ray (Ray): Traced bundle, shape ``[..., num_rays, 3]``, with
                ``stop_weight`` stamped at the aperture stop.
            ray_path (list[torch.Tensor] | None): Optional recorded positions
                to gather alongside the selection.

        Returns:
            chief_ray (Ray): Selected chief ray with diagnostics, shape
                ``[..., 1, 3]``.
            chief_path (list[torch.Tensor] | None): Gathered path entries, or
                None when ``ray_path`` is None.
        """
        if not getattr(ray, "stop_weight_assigned", False):
            raise ValueError(
        ...
        weights = torch.nan_to_num(
            ray.stop_weight.detach(), nan=0.0, posinf=0.0, neginf=0.0
        ).clamp_min(0.0)
        stop_reached = weights.gt(0).any(dim=-1, keepdim=True)
        sample_index = weights.argmax(dim=-1, keepdim=True)
        ...
        chief_ray.stop_weight = gather_scalar(weights)
        chief_ray.shape = chief_ray.o.shape[:-1]

        selected_weight = chief_ray.stop_weight
        safe_weight = selected_weight.clamp_min(
            torch.finfo(selected_weight.dtype).tiny
        )
        residual_normalized = STOP_WEIGHT_SIGMA * torch.sqrt(
    ...
    def _chief_or_centroid_xy(self, ray):
        ...
        if self.aper_idx is None or not getattr(ray, "stop_weight_assigned", False):
            return centroid_xy
